...
def get_suggestions(dictionary, string, silent=False):
	...
	outlist = sorted(as_list, key=lambda a: (a[1][1], -[1][0]))
...
def spell_check_single_word(dictionary, word_in):
	result = []
	doc_word_count = 0.0
	corrected_word_count = 0.0
	unknown_word_count = 0.0
	need_to_correct_word_count = 0.0
	doc_words = preproccess_string(word_in)
	for doc_word in doc_words:
		doc_word_count += 1.0
		if True: #need to test more
			suggestion = get_suggestions(dictionary, doc_word)
			if len(suggestion)==0:
				result.append(doc_word)
				unknown_word_count += 1.0
			else: 
				if suggestion[0] != doc_word:
					result.append(suggestion[0])
					need_to_correct_word_count += 1.0
				else:
					result.append(suggestion[0])
					corrected_word_count += 1.0
	# prob = (corrected_word_count + need_to_correct_word_count) / doc_word_count
	if corrected_word_count / doc_word_count  >= 0.5:
		return " ".join(result) + '(MEDICINE)'
	else:
		return " ".join(result) + '(NOT MEDICINE)' 
...
if __name__ == "__main__":
	...
	while True:
		...
		run_time = time.time() - start_time
		print ('-----')
		print ('%.5f seconds to run' % run_time_)
		print('-----')
